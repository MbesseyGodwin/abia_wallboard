...
jQuery(document).ready(function($) {
    ...
    var jsondata;
    $.ajax({
      url: baseUrl + "GetRiversSurgeTxNewModel",
      ...
      fetchSurgeVlData();
		 // fetchSurgeSummary(); Disabled for ICAP Visit
		  GetSurgeTxCurr();
		  fetchTxNewData();
		  getHtsTesting();
		  getStateYieldData()
...
var ids = //["#hts_chart", "#hts_yield_by_state", "#tx_new_page", "#tx_curr_archeivement", "#vl_summary_page", "#surge_summary_page"];
      ["#hts_chart","#hts_yield_by_state","#tx_new_page","#tx_curr_archeivement","#vl_summary_page"];
    ids.forEach(e => {
        $(e).hide();
    });
    var index = 0;
...
function fetchSurgeVlData() { 
	...
	$.ajax({
	    url: baseUrl + "GetRiversSurgeVLStatistics",
...
{ 
    $.ajax({
        url: baseUrl + "GetTxCurrArchieved/?state=Rivers",
        success: function(data) {
            //console.log(data)   
...
function fetchTxNewData()
{
	...
	let xdata = [];
	$.ajax({
	  url: baseUrl + "GetRiversSurgeTxByWeek",
	  success(data) {
