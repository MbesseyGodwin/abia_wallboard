...
jQuery(document).ready(function($) {
    ...
    var jsondata;
    $.ajax({
      url: baseUrl + "GetFCTSurgeTxNewModel",
...
//var ids = ["#hts_chart","#hts_yield_by_state","#tx_new_page","#tx_curr_archeivement","#vl_summary_page","#surge_summary_page"];
var ids = ["#tx_new_page","#tx_curr_archeivement","#vl_summary_page","#surge_summary_page"];
    ids.forEach(e => {
        $(e).hide();
    });
    var index = 0;
...
function fetchSurgeVlData() { 
	...
	$.ajax({
	    url: baseUrl + "GetSurgeVLStatistics/?state=FCT",
...
function fetchSurgeSummary(){
  ...
  $.ajax({
    ...
    success(data) {
      ...
      $.ajax({
        url: baseUrl + "GetFCTSurgeSummary", 
        success(data) {
          xdata = createNewData(data, htsdata);
          for (i = 0; i < 10; i++) {
            $("#surge_summary_page_body").append(`
          ...
          setInterval(() => {
            $($("#surge_summary_page_body tr")[0]).remove();
...
{ 
    $.ajax({
        url: baseUrl + "GetTxCurrArchieved/?state=FCT",
        success: function(data) {
            //console.log(data)   
    ...
    datasets: [
        ...
        },
        {
          label: 'Target Projection',
            ...
            data: [45085,47208,49332,51455,53579,55702,57825,59949,62072,64196,66319,68442],           
            borderWidth: 1,
            xAxisID: "bar-x-axis1",
            stack: "background"
        }
...
function fetchTxNewData()
{
	...
	let xdata = [];
	$.ajax({
	  url: baseUrl + "GetFCTSurgeTxByWeek", 
	  success(data) {
		...
		for (i = 0; i < 8; i++) {
		  $(".hts").append(`
					  <tr>
						  ...
						  <td style="color:white"><i class="mdi mdi-arrow-${xdata[i].tx_new/xdata[i].avg > 0.8 ? "up" : "down"}-thick ${
		...
		setInterval(() => {
		  ...
		  $(".hts").append(`
			  <tr>
				  ...
				  <td style="color:white"><i class="mdi mdi-arrow-${last.tx_new/last.avg > 0.8 ? "up" : "down"}-thick ${
