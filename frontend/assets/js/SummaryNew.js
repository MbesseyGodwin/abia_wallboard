      ...
      $.ajax({
        ...
        */
          /*
            $("[data-lga]")
              .text(xdata[i].state) 
             // .css({ "border-bottom": `solid 16px ${xdata[i].priority}` });
            $("[data-HTS_TST]").text(xdata[i].hts_tst)
            $("[data-HTS_TST_POS]").text(xdata[i].hts_tst_pos)
            $("[data-TX_New").text(xdata[i].tx_new)
            $("[data-TX_Curr]").text(xdata[i].tx)
         $("[data-PLVS]").text(xdata[i].tx_vls)*/
        }
          ...
          setInterval(() => {
            ...
            /*        $("tbody").append(`
             ...
             `);*/
                 $("[data-lga]").text(xdata[i].state).css({"border-bottom": `solid 16px ${xdata[i].priority}`})
                 //$("[data-HTS_TST]").text(${xdata[i].hts_tst}/${xdata[i].hst_target}<span style="color:#7DCEA0">) 
                 $("[data-HTS_TST]").text(xdata[i].hts_tst)
                 $("[data-HTS_TST_POS]").text(xdata[i].hts_tst_pos)
                 $("[data-TX_New").text(xdata[i].tx_new)
                 $("[data-TX_Curr]").text(xdata[i].tx_curr)
                 $("[data-PLVS]").text(xdata[i].tx_vls) 
